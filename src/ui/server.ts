...
export function startWebUi(opts: StartWebUiOptions): WebServerHandle {
  const server = Bun.serve({
    ...
    fetch: async (req) => {
      ...
      if (url.pathname === "/api/run-channel" && req.method === "POST") {
        if (!opts.onRunChannel) return json({ ok: false, error: "run-channel not configured" });
        try {
          const body = await req.json();
          const channelId = String(body?.channelId ?? "").trim();
          const projectDir = body?.projectDir ? String(body.projectDir).trim() : undefined;
          const prompt = String(body?.prompt ?? "").trim();
          const taskId = body?.taskId ? String(body.taskId).trim() : undefined;
          const taskTitle = body?.taskTitle ? String(body.taskTitle).trim() : undefined;
          if (!channelId) return json({ ok: false, error: "channelId required" });
          if (!prompt) return json({ ok: false, error: "prompt required" });
          // Fire and forget — task runs in background, return immediately
          opts.onRunChannel(channelId, projectDir, prompt, taskId, taskTitle).catch((err) =>
            console.error(`[run-channel] Task error for ${channelId}: ${err}`)
          );
