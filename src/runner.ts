...
/** Active process kill functions, keyed by threadId or '__global__' for non-threaded sessions. */
const activeKillers = new Map<string, () => void>();

/**
 * Stop the currently running Claude process for a given channel/thread.
 * Returns true if a running process was found and killed, false if nothing was running.
 */
export function stopRunningSession(threadId?: string): boolean {
  const key = threadId ?? "__global__";
  const kill = activeKillers.get(key);
...
/**
 * Delete a stale Chrome SingletonLock if the owning process is dead.
 * Prevents "browser locked" errors after a crashed session.
 */
async function clearStaleBrowserLock(projectDir: string): Promise<void> {
  const lockPath = join(projectDir, "browser-profile", "SingletonLock");
  if (!existsSync(lockPath)) return;
  try {
    // SingletonLock is a symlink: "{hostname}-{pid}"
    const target = await import("fs").then(m => m.readlinkSync(lockPath)).catch(() => null);
    if (!target) return;
    ...
    try {
      ...
      await import("fs").then(m => m.unlinkSync(lockPath));
      console.log(`[${new Date().toLocaleTimeString()}] Cleared stale browser lock (PID ${pid} dead)`);
    }
  } catch {
    // ignore any error — lock cleanup is best-effort
  }
}

/**
 * Strip base64 image data from tool_result blocks in session jsonl.
 * Screenshots, PDFs, etc. balloon the transcript — Claude already
 * processed them at capture time, so the accompanying text is enough
 * for resume context.
 */
async function pruneSessionImages(
  projectDir: string,
  sessionId: string
): Promise<{ pruned: number; bytesSaved: number }> {
  const slug = projectDir.replace(/\//g, "-");
  const home = process.env.HOME || "";
  const sessionPath = join(home, ".claude", "projects", slug, `${sessionId}.jsonl`);
  if (!existsSync(sessionPath)) return { pruned: 0, bytesSaved: 0 };

  const originalSize = Bun.file(sessionPath).size;
  ...
  for (const line of lines) {
    ...
    try {
      ...
      // Strip images from message.content (what gets sent to the model)
          ...
          if (block?.type === "tool_result" && Array.isArray(block.content)) {
            for (let i = 0; i < block.content.length; i++) {
              ...
              if (inner?.type === "image" && inner?.source?.type === "base64") {
                const kb = Math.round((inner.source.data?.length || 0) / 1024);
                block.content[i] = {
                  type: "text",
                  text: `[image pruned — ${kb}KB base64 removed]`,
                };
                pruned++;
              }
            }
      ...
      // Strip images from toolUseResult (transcript metadata, bulk of disk usage)
      if (Array.isArray(entry.toolUseResult)) {
        entry.toolUseResult = entry.toolUseResult.filter((item: { type?: string }) => {
          if (item?.type === "image") {
            pruned++;
            return false;
          }
  ...
  if (pruned > 0) {
    ...
    await Bun.write(sessionPath, newText);
    return { pruned, bytesSaved: originalSize - newText.length };
  }
  return { pruned: 0, bytesSaved: 0 };
}
...
async function runClaudeOnce(
  ...
  const killerKey = threadKey ?? "__global__";
  activeKillers.set(killerKey, killFn);
  ...
  try {
    ...
    activeKillers.delete(killerKey);
  }
}
...
async function execClaude(name: string, prompt: string, threadId?: string, projectDir?: string): Promise<RunResult> {
  ...
  // Pass per-channel .mcp.json so each channel gets its own browser profile
  const scopedMcpConfig = join(projectDir ?? PROJECT_DIR, ".mcp.json");
  if (existsSync(scopedMcpConfig)) {
    args.push("--mcp-config", scopedMcpConfig);
  }

  if (!isNew) {
    try {
      const pruneDir = projectDir ?? PROJECT_DIR;
      const pruneResult = await pruneSessionImages(pruneDir, existing.sessionId);
      if (pruneResult.pruned > 0) {
        console.log(
          `[${new Date().toLocaleTimeString()}] Pruned ${pruneResult.pruned} image(s) from session history, saved ${(pruneResult.bytesSaved / 1024 / 1024).toFixed(2)}MB`
        );
      }
    } catch (e) {
      console.error(`[${new Date().toLocaleTimeString()}] Session prune failed:`, e);
    }
  ...
  // Load the project's CLAUDE.md if it exists (use projectDir when channel-scoped)
  const claudeMdPath = projectDir ? join(projectDir, "CLAUDE.md") : PROJECT_CLAUDE_MD;
  const legacyClaudeMdPath = projectDir ? null : LEGACY_PROJECT_CLAUDE_MD;
  const resolvedClaudeMd = existsSync(claudeMdPath) ? claudeMdPath
    : (legacyClaudeMdPath && existsSync(legacyClaudeMdPath)) ? legacyClaudeMdPath
    : null;
  if (resolvedClaudeMd) {
    try {
      const claudeMd = await Bun.file(resolvedClaudeMd).text();
  ...
  const scopeDir = projectDir ?? PROJECT_DIR;

  // Clear stale browser lock if present (prevents locked-profile errors after crash)
  await clearStaleBrowserLock(scopeDir);
  const dirScopePrompt = [
    `CRITICAL SECURITY CONSTRAINT: You are scoped to the project directory: ${scopeDir}`,
    "You MUST NOT read, write, edit, or delete any file outside this directory.",
    "You MUST NOT run bash commands that modify anything outside this directory (no cd /, no /etc, no ~/, no ../.. escapes).",
    "If a request requires accessing files outside the project, refuse and explain why.",
  ].join("\n");
...
export async function run(name: string, prompt: string, threadId?: string, projectDir?: string): Promise<RunResult> {
  return enqueue(() => execClaude(name, prompt, threadId, projectDir), threadId);
}
...
export async function runUserMessage(name: string, prompt: string, threadId?: string, projectDir?: string): Promise<RunResult> {
  return run(name, prefixUserMessageWithClock(prompt), threadId, projectDir);
}
