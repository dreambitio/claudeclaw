...
export interface Job {
  ...
  /** Discord channel ID to post results to (and use as threadId for session scoping) */
  channelId?: string;
  /** Project directory to run the job in (uses channel's project dir by default when channelId is set) */
  projectDir?: string;
}
...
function parseJobFile(name: string, content: string): Job | null {
  ...
  const channelIdLine = lines.find((l) => l.startsWith("channelId:"));
  const channelId = channelIdLine
    ? parseFrontmatterValue(channelIdLine.replace("channelId:", ""))
    : undefined;

  const projectDirLine = lines.find((l) => l.startsWith("projectDir:"));
  const projectDir = projectDirLine
    ? parseFrontmatterValue(projectDirLine.replace("projectDir:", ""))
    : undefined;

  return { name, schedule, prompt, recurring, notify, channelId, projectDir };
}
