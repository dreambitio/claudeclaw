...
async function sendPhotoToChat(
  ...
  const file = Bun.file(filePath);
  if (!(await file.exists())) {
    console.error(`[Telegram] sendPhoto: file not found: ${filePath}`);
    return;
  }
...
async function sendLocalFile(
  ...
  if (IMAGE_EXTENSIONS.has(ext)) {
    await sendPhotoToChat(token, chatId, filePath, threadId);
  } else {
    await sendDocumentToChat(token, chatId, filePath, threadId);
  }
}
  ...
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      for (const fp of filePaths) {
        try {
          await sendLocalFile(config.token, chatId, fp, threadId);
        } catch (err) {
