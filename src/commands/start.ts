...
export async function start(args: string[] = []) {
  ...
  let discordPostTaskAnnouncement: ((channelId: string, taskId: string, taskTitle: string) => Promise<{ announcementMessageId: string; threadId: string }>) | null = null;
  let discordToken = "";
  ...
  async function initDiscord(token: string) {
    if (token && token !== discordToken) {
      ...
      discordPostTaskAnnouncement = null;
      discordToken = "";
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      try {
        return startWebUi({
          ...
          onRunChannel: async (channelId, projectDir, prompt, taskId, taskTitle) => {
            let targetChannelId = channelId;
            if (taskId && taskTitle && discordPostTaskAnnouncement) {
              try {
                const { threadId } = await discordPostTaskAnnouncement(channelId, taskId, taskTitle);
                targetChannelId = threadId;
              } catch (err) {
                console.error(`[Discord] Failed to create task thread for ${taskId}: ${err}`);
              }
            }
            const result = await runUserMessage(`run-channel-${channelId}`, prompt, targetChannelId, projectDir);
            if (discordSendToChannel) {
              const text = result.exitCode === 0
                ? result.stdout || "(empty)"
                : `Error (exit ${result.exitCode}): ${result.stderr || "Unknown"}`;
              discordSendToChannel(targetChannelId, text).catch((err) =>
                console.error(`[Discord] Failed to post run-channel result to ${targetChannelId}: ${err}`)
              );
            }
            return result;
          },
    ...
    for (const job of currentJobs) {
      if (cronMatches(job.schedule, now, currentSettings.timezoneOffsetMinutes)) {
        resolvePrompt(job.prompt)
          .then((prompt) => {
            if (job.channelId) {
              // Run in channel-scoped session with the channel's project directory
              const projectDir = job.projectDir ?? currentSettings.discord?.channelProjects?.[job.channelId];
              return runUserMessage(job.name, prompt, job.channelId, projectDir);
            }
            return run(job.name, prompt);
          })
          .then((r) => {
            if (job.notify === false) return;
            if (job.notify === "error" && r.exitCode === 0) return;
            // If job has a channelId, post to that channel; otherwise use DM/Telegram
            if (job.channelId && discordSendToChannel) {
              const text = r.exitCode === 0
                ? r.stdout || "(empty)"
                : `error (exit ${r.exitCode}): ${r.stderr || "Unknown"}`;
              discordSendToChannel(job.channelId, text).catch((err) =>
                console.error(`[Discord] Failed to post job result to channel ${job.channelId}: ${err}`)
              );
            } else {
